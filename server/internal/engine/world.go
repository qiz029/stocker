...
type World struct {
	...
	Forum      []ForumPost // NPC 论坛帖, 按 Day 升序; 入库 room_forum_posts
}
...
func GenerateWorld(sc *scenario.Scenario, seed uint64) (*World, error) {
	...
	news = append(news, RecapNews(sc, prices, seed)...)
	FillFallbackCopy(sc, news, seed)
	sort.SliceStable(news, func(i, j int) bool { return news[i].Day < news[j].Day })
	forum := ForumPosts(sc, prices, news, seed)
	return &World{ScenarioID: sc.ID, Seed: seed, Prices: prices, News: news, Forum: forum}, nil
}
